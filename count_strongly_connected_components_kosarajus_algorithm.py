"""
Count Strongly Connected Components (Kosaraju’s Algorithm)

You are given an unweighted directed graph having 'V' vertices and 'E' edges. Your task is to count the number of strongly connected components (SCCs) present in the graph.
A directed graph is said to be strongly connected if every vertex is reachable from every other vertex. The strongly connected components of a graph are the subgraphs which are themselves strongly connected.

Note :
Use zero-based indexing for the vertices.
The given graph doesn’t contain any self-loops.

Sample Input 1 :
1
5 6
0 1
1 2
1 4
2 3
3 2
4 0
Sample Output 1 :
2
Explanation of sample input 1 :
For the first test case, the graph is shown below. There are two SCCs in the graph, which are enclosed in the boxes as shown in the image below.

Sample Input 2 :
2
1 0
4 4
0 1
1 2
2 3
3 1
Sample Output 2 :
1
2

Constraints :
1 <= T <= 10
1 <= V <= 10^4
0 <= E <= 10^4
0 <= a, b < V

Time Limit: 1 sec

LINK : https://www.naukri.com/code360/problems/count-strongly-connected-components-kosaraju-s-algorithm_1171151?leftPanelTabValue=PROBLEM
"""

# Optimal - Kosaraju’s two-pass DFS.
"""
    Time complexity     : ( V + E )
    Space complexity    : ( V + E )
"""
from collections import defaultdict

def stronglyConnectedComponents(v, edges):
    # Create adjacency list
    adj_matrix = defaultdict(list)
    for source, dest in edges:
        adj_matrix[source].append(dest)

    # Initialize visited array to accommodate all nodes
    visited = [False] * v

    # DFS to fill order of nodes in stack
    def dfs(node, visited, stack):
        visited[node] = True
        for neighbour in adj_matrix[node]:
            if not visited[neighbour]:
                dfs(neighbour, visited, stack)
        stack.append(node)

    # Transpose the graph
    def transpose():
        transposed = defaultdict(list)
        for node in adj_matrix:
            for neighbour in adj_matrix[node]:
                transposed[neighbour].append(node)
        return transposed

    # DFS for SCC
    def dfs_count(node, visited, component):
        visited[node] = True
        component.append(node)
        for neighbour in transpose_matrix[node]:
            if not visited[neighbour]:
                dfs_count(neighbour, visited, component)

    stack = []
    for i in range(v):
        if not visited[i]:
            dfs(i, visited, stack)

    transpose_matrix = transpose()
    visited = [False] * v
    sccs = []

    while stack:
        node = stack.pop()
        if not visited[node]:
            component = []
            dfs_count(node, visited, component)
            sccs.append(component)

    return len(sccs)
